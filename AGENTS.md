...
Symphony configuration files are split between local and production environments:
- For local testing and development, use the workflows in the `./local_workflows/` directory. They contain local database IDs (e.g. `account_id`, `board_id`) specifically generated by `bin/rails db:seed`. To start the runner locally: `SYMPHONY_WORKFLOW_PATH=local_workflows bin/rails symphony:run`.
- For changes dedicated to the production/remote version (deployed via Kamal), update the files inside the `./workflows/` directory.

## smoketest symphony
For changes made to Symphony, run the web app locally and run Symphony locally against `local_workflows/WORKFLOW.local.md`.

Recommended smoketest flow:
- Start the app with `bin/dev` and verify `http://fizzy.localhost:3006` loads.
- Start Symphony with `SYMPHONY_WORKFLOW_PATH=local_workflows bin/rails symphony:run`.
- Open board `testboard` (`03fsfyojf2wdh7l7peehqce3x`) in the `cleanslate` account.
- Create two cards with unique timestamps in the title/body. Each card should instruct the agent to append the timestamp to `README.md` and create `hello_<timestamp>.py` that prints `hello<timestamp>`.
- Move both new cards into the `Todo` column. The workflow is configured with `agent.max_concurrent_agents: 2`, so both cards should be claimed in the same polling cycle when two slots are free.
- Verify behavior in three places:
  1. board state in the UI (`Todo` -> `In Progress` -> `Review`),
  2. runner log output, and
  3. Symphony telemetry log.

Useful local evidence to inspect:
- Runner log: `log/symphony-run*.log`
- Workspace root used by the workflow: `tmp/symphony_workspaces/`
- Symphony telemetry log default: `/tmp/symphony_workspaces/telemetry.log`
- Telemetry summary helper: `ruby script/summarize_symphony_telemetry.rb /tmp/symphony_workspaces/telemetry.log`

Telemetry expectations for a successful parallel smoketest:
- one stable `trace_id` per card,
- distinct `span_id` values per event,
- `symphony.issue.pickup`, `symphony.issue.transition`, `symphony.workspace.checkout`, `symphony.agent.start`, `symphony.agent.command.start`, `symphony.agent.command.finish`, `symphony.agent.finish`, and `symphony.pull_request.created` events,
- interleaved timestamps from the two cards showing concurrent execution.

Rework behavior to verify when testing rework flows:
- If a card appears in the `Rework` column, Symphony should still pick it up and move it to `In Progress`.
- The agent should read both the card comments and the GitHub PR comments, summarize the requested changes, and then implement them.
- For rework, Symphony should reuse the previously associated branch instead of creating a brand new branch.
- After the rework implementation finishes successfully, Symphony should update the existing branch, refresh the PR state/comments, and move the card onward the same way as a normal successful run.

Practical tip: the telemetry log path is not workspace-relative by default; it comes from `Dir.tmpdir`, so check `/tmp/symphony_workspaces/telemetry.log` unless the workflow overrides `telemetry.log_path`.
